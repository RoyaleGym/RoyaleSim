...
on exactly F+7 and none on F+1 to F+6. The Golemite row has no DeployDelay, so the rule is not DeployDelay. Today's
engine lets an idle Cannon target the Brawler and a Golemite on F+1.

WHY THE CONTROL IS HERE. A hand-played unit is targeted on its first frame, on client 15.535.29 (a Barbarian beside the
Goblin Cage scenario) and in today's engine (a Knight). An implementation that delays every new unit fails it.
"""
...
NEW_ARM, OLD_ARM = "client_8th_frame", "none"
...
def test_brawler_first_targeted_on_its_8th_frame():
    brawler, first, looks, names = cage_run(NEW_ARM)
...
def test_golemites_first_targeted_on_their_8th_frame():
    ...
    kids, first, looks, names = golem_run(NEW_ARM)
...
def test_old_arm_is_todays_engine():
    brawler, first, looks, names = cage_run(OLD_ARM)
    got = looks.get((brawler, cannon_uid(names)))
    assert offset(got, first) == 1, f"old arm: the idle Cannon targeted the Brawler on F+{offset(got, first)}, not F+1"
