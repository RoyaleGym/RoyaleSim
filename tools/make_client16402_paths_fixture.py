...
WHERE THE INPUTS ARE
    ...
    the real game (CR 16.402). Two variables name them, neither with a default:
      ...
      ROYALELIVE_SAMPLER  the module that builds cases from a capture, as a filesystem path
...
WHAT IT HOLDS
    ...
    BOTH sides alive on that tick and the published node list, goal first. Each building is
    [x, y, CollisionRadius, side]: its absolute native centre, its radius, and the side that owns
    it, on the same 0/1 scale as the case's own `side`. The mover's own buildings come first. The
    owner is what lets the gate run the friendly-only control arm (a building is friendly when
    its side equals the case's). The Rust gate (tests/oracle2026.rs
...
# guesses it: it is a path, given outright. If it sits in a package, import it under the name
# the package gives it, so its own imports resolve.
SAMPLER = os.environ.get("ROYALELIVE_SAMPLER")
if not SAMPLER:
    sys.exit("set ROYALELIVE_SAMPLER to the path of the module that builds cases from a capture")
_file = pathlib.Path(SAMPLER).resolve()
if not _file.is_file():
    sys.exit(f"no module at {_file} (ROYALELIVE_SAMPLER)")
_parts = [_file.stem]
_root = _file.parent
...
def build() -> dict:
    ...
    for group, samples in (("live_16402", live), ("offline_15535_lane_sweep", offline)):
        for s in samples:
            # [x, y, r, owner side]: the sampler hands over the mover's own buildings and the
            # other side's as two lists, and the owner must survive the join.
            if s.side not in (0, 1):
                raise SystemExit(f"{s.name}: side {s.side!r} is not 0 or 1")
            own = [[x, y, r, s.side] for (x, y, r) in s.occluders]
            enemy = [[x, y, r, 1 - s.side] for (x, y, r) in s.enemy_occluders]
            occl = own + enemy
    ...
    return {
        "$comment": "GENERATED by tools/make_client16402_paths_fixture.py from the live trace corpus "
                    ...
                    "occluders_native entries are [x, y, CollisionRadius, owner side], the side on the same "
                    "0/1 scale as the case's side.",
        "cases": cases,
    }
