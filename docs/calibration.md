...
A re-read on 2026-09-22 went through 24 of the 180 entries against the corpus. It moved no
...
156 entries have not been re-read. So, concretely (every count on this page comes from
...
- **The shape of an entry is worth trusting where a judgement was made, but it is not
  ...
  and 90 measured. The tools agree on 180 by convention, and the convention undercounts by one. 126 name the rivals the
...
disagree. Of the 180 top-level keys with a status, 89 are `measured`; a 181th entry nested inside another is measured too.
