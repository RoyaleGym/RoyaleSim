...
A re-read on 2026-09-22 went through 24 of the 175 entries against the corpus. It moved no
...
151 entries have not been re-read. So, concretely (every count on this page comes from
...
- **The shape of an entry is worth trusting where a judgement was made, but it is not
  universal, so check rather than assume.** All 175 carry a status. That 175 counts TOP-LEVEL
  entries; one further entry, `pathfinding.PATHFINDING_COSTS.application`, is nested inside
  another and carries its own `measured` status, so counting every status in the file gives 176
  and 82 measured. The tools agree on 175 by convention, and the convention undercounts by one. 121 name the rivals the
  value was chosen against and 129 state what would move it, and 113 do both. The gap is mostly
...
disagree. Of the 175 top-level keys with a status, 81 are `measured`; a 176th entry nested inside another is measured too.
