...
A re-read on 2026-09-22 went through 24 of the 165 entries against the corpus. It moved no
...
141 entries have not been re-read. So, concretely (every count on this page comes from
...
- **The shape of an entry is worth trusting where a judgement was made, but it is not
  universal, so check rather than assume.** All 165 carry a status. That 165 counts TOP-LEVEL
  entries; one further entry, `pathfinding.PATHFINDING_COSTS.application`, is nested inside
  another and carries its own `measured` status, so counting every status in the file gives 166
  and 75 measured. The tools agree on 165 by convention, and the convention undercounts by one. 112 name the rivals the
  value was chosen against and 120 state what would move it, and 104 do both. The gap is mostly
...
disagree. Of the 165 top-level keys with a status, 74 are `measured` and one is an `owner_ruling`; a 166th entry nested inside another is measured too.
