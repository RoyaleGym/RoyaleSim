...
- **The shape of an entry is worth trusting where a judgement was made, but it is not
  ...
  and 75 measured. The tools agree on 168 by convention, and the convention undercounts by one. 115 name the rivals the
...
disagree. Of the 168 top-level keys with a status, 74 are `measured` and one is an `owner_ruling`; a 169th entry nested inside another is measured too.
