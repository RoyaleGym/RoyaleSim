...
A re-read on 2026-09-22 went through 24 of the 168 entries against the corpus. It moved no
...
144 entries have not been re-read. So, concretely (every count on this page comes from
...
- **The shape of an entry is worth trusting where a judgement was made, but it is not
  universal, so check rather than assume.** All 168 carry a status. That 168 counts TOP-LEVEL
  entries; one further entry, `pathfinding.PATHFINDING_COSTS.application`, is nested inside
  another and carries its own `measured` status, so counting every status in the file gives 169
  and 80 measured. The tools agree on 168 by convention, and the convention undercounts by one. 115 name the rivals the
  value was chosen against and 123 state what would move it, and 107 do both. The gap is mostly
  ...
  the 79 `measured` entries name no rival at all, and 12 of those state no promotion criterion
...
disagree. Of the 168 top-level keys with a status, 79 are `measured` and one is an `owner_ruling`; a 169th entry nested inside another is measured too.
