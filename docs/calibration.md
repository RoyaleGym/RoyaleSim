...
- **The shape of an entry is worth trusting where a judgement was made, but it is not
  ...
  value was chosen against and 111 state what would move it, and 94 do both. The gap is mostly
  ...
  the 64 `measured` entries name no rival at all, and 12 of those state no promotion criterion
...
disagree. Of the 156 top-level keys with a status, 64 are `measured` and one is an `owner_ruling`; a 156th entry nested inside another is measured too.
