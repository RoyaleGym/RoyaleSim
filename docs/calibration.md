...
A re-read on 2026-09-22 went through 24 of the 182 entries against the corpus. It moved no
...
158 entries have not been re-read. So, concretely (every count on this page comes from
...
- **The shape of an entry is worth trusting where a judgement was made, but it is not
  universal, so check rather than assume.** All 182 carry a status. That 182 counts TOP-LEVEL
  entries; one further entry, `pathfinding.PATHFINDING_COSTS.application`, is nested inside
  another and carries its own `measured` status, so counting every status in the file gives 183
  and 90 measured. The tools agree on 182 by convention, and the convention undercounts by one. 128 name the rivals the
  value was chosen against and 136 state what would move it, and 120 do both. The gap is mostly
...
disagree. Of the 182 top-level keys with a status, 89 are `measured`; a 183th entry nested inside another is measured too.
