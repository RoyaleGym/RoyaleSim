...
A re-read on 2026-09-22 went through 24 of the 183 entries against the corpus. It moved no
...
159 entries have not been re-read. So, concretely (every count on this page comes from
...
- **The shape of an entry is worth trusting where a judgement was made, but it is not
  universal, so check rather than assume.** All 183 carry a status. That 183 counts TOP-LEVEL
  entries; one further entry, `pathfinding.PATHFINDING_COSTS.application`, is nested inside
  another and carries its own `measured` status, so counting every status in the file gives 184
  and 91 measured. The tools agree on 183 by convention, and the convention undercounts by one. 129 name the rivals the
  value was chosen against and 137 state what would move it, and 121 do both. The gap is mostly
  ...
  the 90 `measured` entries name no rival at all, and 13 of those state no promotion criterion
...
disagree. Of the 183 top-level keys with a status, 90 are `measured`; a 184th entry nested inside another is measured too.
