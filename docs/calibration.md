...
A re-read on 2026-09-22 went through 24 of the 181 entries against the corpus. It moved no
...
157 entries have not been re-read. So, concretely (every count on this page comes from
...
- **The shape of an entry is worth trusting where a judgement was made, but it is not
  universal, so check rather than assume.** All 181 carry a status. That 181 counts TOP-LEVEL
  entries; one further entry, `pathfinding.PATHFINDING_COSTS.application`, is nested inside
  another and carries its own `measured` status, so counting every status in the file gives 182
  and 90 measured. The tools agree on 181 by convention, and the convention undercounts by one. 127 name the rivals the
  value was chosen against and 135 state what would move it, and 119 do both. The gap is mostly
  ...
  the 89 `measured` entries name no rival at all, and 13 of those state no promotion criterion
...
disagree. Of the 181 top-level keys with a status, 89 are `measured`; a 182th entry nested inside another is measured too.
