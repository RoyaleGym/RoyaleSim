...
| `cards-and-spells.svg` | video placeholder | 144 cards, towers, spells, overtime - a Fireball landing on a crowd beside a princess tower, the tower's hp bar dropping, then the clock entering overtime and a 3-crown finish |
...
| `measured-routes.svg` | image placeholder | Routes measured off real battles - one deploy, two routes on one board: the recorded 16.402 route and the engine's, node for node, plus the 743 of 744 score |
