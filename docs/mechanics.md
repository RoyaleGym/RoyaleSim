...
| Periodic spawners, death spawn | `state.rs::spawner_pass`, `state.rs::phase_reap`, `card.rs::SpawnerDef` / `DeathSpawnDef` | the `Spawn*` / `DeathSpawn*` columns (huts, Witch, Dark Witch; Tombstone, Golem, Lava Hound, Battle Ram): waves on the data's cadence, each unit created in the tick its timer runs out, stun pauses the timer, `SpawnLimit` counts the spawner's own live units, death spawns on a ring of `DeathSpawnRadius` on the dying unit's facing (`spawner.DEATH_SPAWN_LAYOUT = facing_ring`, measured), except the Tombstone's and the Barbarian Hut's, which appear together on the point the spawner emits at (`spawner.DEATH_SPAWN_AT_EMISSION_POINT`). Eleven `spawner.*` keys are measured: FIRST_WAVE, START_TIME_ORIGIN, TIMER_LEFTOVER, SPAWN_POINT, DEATH_SPAWN_RADIUS_DEFAULT, DEATH_SPAWN_LAYOUT, DEATH_SPAWN_AT_EMISSION_POINT, DEATH_SPAWN_DEPLOY_TIME_DEFAULT, EMISSION_TIMING, RELEASE_TIMING, SPAWNED_DEPLOY_TIME; PAUSE_ANCHOR is a hypothesis, STUN_PAUSES_SPAWNER is datamined, LIMIT_RULE is a guess. DEATH_SPAWN_PUSHBACK is a hypothesis that ships at `not_read` (see "Not modelled"). `tests/spawner.rs`, `tests/death_spawn_pushback.rs` |
...
| A death spawn's first frames | the shipped value of `targeting.SPAWNED_UNIT_ACQUIRE_DELAY` (`none`) lets an enemy target a death spawn's troops from the tick after they appear. In the game no enemy targets one before its 8th frame (measured on client 15.535.29, the Golemites and a Goblin Cage's Brawler among them). The other value, `client_8th_frame`, runs that law and is not shipped yet. In the game a hand-played troop, a Tombstone's periodic Skeletons and the Goblin Drill's building are targeted at once (measured on client 15.535.29). The engine also exempts every other periodic spawner's troops, the Barbarian Hut's and the Witch's among them, and any building a death spawn creates. That part is not measured, and the Barbarian Hut is an open case: the Goblin Hut's waves do wait in the game. Under `client_8th_frame` a hidden Tesla does not rise for a unit inside its delay, and area damage still hits the unit. Neither is measured. `tests/spawn_acquire_delay.rs` |
