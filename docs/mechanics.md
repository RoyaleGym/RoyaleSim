...
| Melee and ranged attacks | `combat.rs`, `state.rs::phase_target` | windup, cooldown, splash, `self_as_aoe_center`. Integer damage; the rounding law is a guess (`combat.DAMAGE_ARITHMETIC`). **A kill does not cost the killer its cycle** (`combat.RETARGET_PROGRESS = keep_when_dead`, measured): replacing a target that DIED keeps the attack progress, so a shooter facing one-shot victims fires on its own `HitSpeed`; switching away from a target still ALIVE still cancels the swing. Under the earlier `reset_always` a princess tower killing skeletons reloaded in 19 ticks against its own 16, once per target change and so once per shot against a swarm. `tests/test_retarget_cadence.py` |
