...
with the truth's, then counts **unit-ticks**: one matched pair (or one unmatched entity) on one
truth frame tick where at least one side has the unit alive. A position column (`<=250`, `<=500`,
...
the isolated-walk subset — truth walking at a tower or at nothing, full hitpoints — and
`walk <=20` is the bit-exact walk: a unit whose slowest step is 37 native per tick is either on
the game's own integer path or it is not.
...
the decks with the cards the loader refuses) but they are also the shorter half, so cutting them
...
Tombstone, at 20.7 %, is the extreme case — a building that is itself stationary and correct,
scored through the Skeletons it emits, whose spawn points and spawn ticks the engine does not yet
place where the game places them.

## 5. First divergence, by cause

Each battle's **first divergence** is its first unit-tick past 1000 native or its first alive
mismatch, and the harness reads a cause at the onset — the first tick that unit's error passed
250. One cause per battle, so the table below ranks what goes wrong *first*, not what costs most
